...
int main(int argc, char *argv[])
{
    ...
    for(i=1;i<argc;i++)
    {
        ...
        else if (!strncmp (argv[i], "-pmode", 3)) 
        {
            ...
            else if(argv[i][0]=='i') pathomode = 'i';
            else
            {
    ...
    if(layeronly=='n')
    {
        ...
        if(strcmp(pathoname,""))
        {
            ...
            if(pathomode=='e')
            {
                if(VipMerge(vol, patho, VIP_MERGE_ALL_TO_ONE, 0, 0)==PB) return(VIP_CL_ERROR);
            }
            else
            {
    ...
    if(patho)
    {
        ...
        {
            if(VipMerge(vol, patho, VIP_MERGE_ALL_TO_ONE, 0, 0)==PB) return(VIP_CL_ERROR);
            if(VipConnexVolumeFilter(vol, CONNECTIVITY_26, -1, CONNEX_BINARY)==PB) return(PB);
        }
        else
        {
            if(VipMerge(vol, patho, VIP_MERGE_ALL_TO_ONE, 0, 255)==PB) return(VIP_CL_ERROR);
            if(VipConnexVolumeFilter(vol, CONNECTIVITY_26, -1, CONNEX_BINARY)==PB) return(PB);
        }
        VipFreeVolume(patho);
    } 
...
static int Usage()
{
    ...
    (void)fprintf(stderr,"        [-pm[ode] {char e/i: exclude/include, default:e}]\n");
...
static int Help()
{
    ...
    (void)printf("add a mask of the lesion to be taken into account\n");
    (void)printf("This can solve bad morphological process\n");
    (void)printf("        [-pm[ode] {char e/i: choice between exclude or include the mask of a lesion in the brain mask, default:e}]\n");
