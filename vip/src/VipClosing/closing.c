 ...
 * MODULE NAME : VipClosing.c         * TYPE     : Command line
...
{     
  ...
  VIP_DEC_VOLUME(vol); 
  char *input = NULL;
  char output[VIP_NAME_MAXLEN] = "closed";
  int xmasksize = 3;
  int ymasksize = 3;
  int zmasksize = 3;
  float size = 0.;
  ...
  if (input==NULL)
    {
      VipPrintfError("input arg is required by VipClosing");
      return(Usage());
    }
 ...
 if(connectivity_activ==VTRUE)
  ...
  if(size<=0.)
    {
      VipPrintfError("Structuring element size is required by VipClosing (mm.)");
      return(Usage());
    }
  ...
  if(connectivity_activ==VFALSE)
    {
      if (VipCustomizedChamferClosing (vol, size, xmasksize, ymasksize, zmasksize, multfactor, algodefine)==PB) return(VIP_CL_ERROR);
    }
  else
    {
      if (VipConnectivityChamferClosing (vol, size, connectivity, algodefine)==PB) return(VIP_CL_ERROR);
    }
...
static int Usage()
{
  (void)fprintf(stderr,"Usage: VipClosing\n");
  (void)fprintf(stderr,"        -i[nput] {image name}\n");
  (void)fprintf(stderr,"        -s[ize] {closing size in mm.}\n");
  (void)fprintf(stderr,"        [-a[lgo] {char: f or t (default:\"f\")}]\n");
  (void)fprintf(stderr,"        [-o[utput] {image name (default:\"closed\")}]\n");
