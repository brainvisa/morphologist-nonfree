	  ...
	  if(buffer[1]=='H')
		      ...
		      &&(search==(buffer+1)))
		{
		  ...
		  if(((n_type_list==1)&&(extern_static_flag[0]==EXTERN))||(n_type_list==2))
		    {
		      (void)strcpy(command,"cat");
		      for(i=0;i<func_ntype;i++)
			{
			  if(extern_static_flag[i]==EXTERN)
			    {
			      (void)strcat(command," ");
			      (void)strcat(command,temp_hfilename[i]);
			    }
			}
		      (void)strcat(command," >> ");
		      (void)strcat(command,externh_filename);
		  ...
		  if(((n_type_list==1)&&(extern_static_flag[0]==STATIC))||(n_type_list==2))
		      ...
		      for(i=0;i<func_ntype;i++)
			{
		      ...
		      (void)strcat(command," >> ");
		      (void)strcat(command,statich_filename);
...

		{
		  ...
		  if(body_count==FALSE)
			...
			{
			  (void)strcat(command," ");
			  (void)strcat(command,temp_filename[i]);
			}
		      (void)strcat(command," >> ");
		      (void)strcat(command,c_filename);
		      system(command);
