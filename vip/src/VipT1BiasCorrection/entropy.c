...
/*---------------------------------------------------------------------------*/
Volume *VipComputeT1BiasFieldOneGrid(int mode, int dumb, Volume *vol, Volume *crest, float undersampling, float Kentropy,
                                     float Kregularization, float Kcrest, float Koffset, float amplitude,
...
/*---------------------------------------------------------------------------*/
Volume *VipComputeT1BiasFieldMultiGrid(int mode, int dumb, Volume *vol, Volume *crest, float undersampling, 
				     float Kentropy,
...
/*---------------------------------------------------------------------------*/
{
  ...
  while(ngrid--)
    {
      field = VipComputeT1BiasFieldOneGrid(mode, dumb, vol, crest, undersampling, Kentropy,
                                           Kregularization, Kcrest, Koffset, amplitude, 
...
total_average denotes the average including field (with averaging) */
  ...
  if(mode==GEOMETRY)
    {
      averageW = ComputeAverageField(vol,crest,255,field,xundersampling,yundersampling,zundersampling);
      if(averageW==PB) return(PB);
      averageW_first = VipGetDataPtr_VFLOAT( averageW ) + field_vos->oFirstPoint;

      squareW = ComputeSquareField(vol,crest,255,field,xundersampling,yundersampling,zundersampling, &total_averageW, &total_squareW, &init_sdW, &volsizeW);  
  ...
  if(dumb==VFALSE)
    {
      ...
      printf("Uentropy: %f, Uregul: %f, Uoffset: %f, Ucrest: %f, Energy: %f, inc:%f\n",entropy*Kentropy,R,O,OG,U,increment);
    }
  else
    {
			...
			{ 
			  ...
			  deltaU[i] += deltaR[i];
                          ...
                          if (mode==GEOMETRY)
                          {
                            ...
                            deltaOG[i] = Kcrest*newsW*newsW - OG;
                            deltaU[i] += deltaOG[i];
                          }
