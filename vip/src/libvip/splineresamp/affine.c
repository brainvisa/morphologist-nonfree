...
static	int			directBsplineMirror
		...
		p = allocLinF(nx);
		if (p == (float *)NULL) {
		  VipPrintfError("ERROR - Not enough memory in directBsplineMirror");
		  return(ERROR);
		}
		for (z = 0L; (z < nz); z++)
		  for (y = 0L; (y < ny); y++) {
		...
		switch (degree) {
		  ...
		  case 7:
			...
			break;
		  default:
			VipPrintfError("ERROR - Unknown degree");
			return(ERROR);
		}
		if (nx > 1L) {
		  data = allocLinD(nx);
		  if (data == (double *)NULL) {
			VipPrintfError("ERROR - Unable to allocate x data");
			return(ERROR);
		  }
		  for (z = 0L; (z < nz); z++)
			for (y = 0L; (y < ny); y++) {
		...
		if (ny > 1L) {
		  data = allocLinD(ny);
		  if (data == (double *)NULL) {
			VipPrintfError("ERROR - Unable to allocate y data");
			return(ERROR);
		  }
		  for (z = 0L; (z < nz); z++)
			for (x = 0L; (x < nx); x++) {
		...
		if (nz > 1L) {
		  data = allocLinD(nz);
		  if (data == (double *)NULL) {
			VipPrintfError("ERROR - Unable to allocate z data");
			return(ERROR);
		  }
		  for (y = 0L; (y < ny); y++)
			for (x = 0L; (x < nx); x++) {
			...
			if (ipiv[j] != 1)
			  for (k = 0; (k < n); k++) {
				if (ipiv[k] == 0) {
				  ...
				  VipPrintfError("ERROR - Singular matrix in Gauss inversion");
				  return(ERROR);
				}
			  }
		  ++(ipiv[icol]);
...
static	int			invertTrsf		(struct trsfStruct	*dirTrsf,
		...
		if (gaussj(dirSkew, invTrsf->skew) == ERROR) {
		  VipPrintfError("ERROR - Non reversible transformation");
		  return(ERROR);
		}
		for (j = 0; (j < 3); j++) {
		  invTrsf->dx[j] = 0.0;
									 ...
									 float				background) 
{
		...
		if ((transform[3][0] != 0.0) || (transform[3][1] != 0.0) || (transform[3][2] != 0.0)
		  || (transform[3][3] != 1.0)){
		  VipPrintfError("ERROR - Non-homogeneous transformation");
		  return(ERROR);
		}
		...
		if (invertTrsf(&trsf, &invTrsf) == ERROR) {
		  VipPrintfError("ERROR - Unable to compute the backward transformation");
		  return(ERROR);
		}
		/*invTrsf = trsf;*//*JEFF*/
		...
		switch (interpolationDegree) {
			  ...
			  for (yOut = -yO; (yOut < ((double)outny - yO)); yOut += 1.0) {
				...
				for (xOut = -xO; (xOut < ((double)outnx - xO)); xOut += 1.0) {
				  ...
				  if ((zIn < 0.0) && ((double)z != zIn))
					z--;
				  if ((0L <= x) && (x < innx) && (0L <= y) && (y < inny) && (0L <= z) && (z < innz))
					*outPtr++ = *(inPtr + (ptrdiff_t)(z * nxy + y * innx + x));
				  else
					*outPtr++ = background;
				}
			  }
		  ...
		  case 1:
			  ...
			  for (yOut = -yO; (yOut < ((double)outny - yO)); yOut += 1.0) {
				...
				for (xOut = -xO; (xOut < ((double)outnx - xO)); xOut += 1.0) {
				  ...
				  if ((0L <= x) && (x < innx) && (0L <= y) && (y < inny) && (0L <= z) && (z < innz)) {
					...
					fy0 = (ptrdiff_t)(fold(y, inny) * innx);
					y++;
					wy1 = BsplnWght1(y, yIn);
					fy1 = (ptrdiff_t)(fold(y, inny) * innx);
					xIn -= 0.5;
					x = (long)xIn;
					if ((xIn < 0.0) && ((double)x != xIn))
					  x--;
					wx0 = BsplnWght1(x, xIn);
					fx0 = (ptrdiff_t)fold(x, innx);
					x++;
					wx1 = BsplnWght1(x, xIn);
					fx1 = (ptrdiff_t)fold(x, innx);
					if (innz == 1L) {
					  pj = inPtr;
					  pi = pj + fy0;
					  ...
					  if ((zIn < 0.0) && ((double)z != zIn))
						z--;
					  pj = inPtr + (ptrdiff_t)(fold(z, innz) * nxy);
					  pi = pj + fy0;
					  qi = wx0 * (double)*(pi + fx0);
					  qi += wx1 * (double)*(pi + fx1);
					  qj = wy0 * qi;
			...
			break;
		  case 2:
			splinePtr = allocVolF(innx, inny, innz);
			if (splinePtr == (float *)NULL) {
			  VipPrintfError("ERROR - Not enough memory for B-spline coefficients");
			  return(ERROR);
			}
			if (directBsplineMirror(inPtr, splinePtr, innx, inny, innz, 2) == ERROR) {
			  freeVolF(splinePtr);
			  VipPrintfError("ERROR - Unable to compute B-spline coefficients");
			  return(ERROR);
			}
			for (zOut = -zO; (zOut < ((double)outnz - zO)); zOut += 1.0) {
			  xz = invTrsf.skew[0][2] * zOut + invTrsf.dx[0] + xO;
				...
				for (xOut = -xO; (xOut < ((double)outnx - xO)); xOut += 1.0) {
				  ...
				  if ((0L <= x) && (x < innx) && (0L <= y) && (y < inny) && (0L <= z) && (z < innz)) {
					...
					for (j = y; (j < (y + width)); j++) {
					  ...
					  *fdy++ = (ptrdiff_t)(fold(j, inny) * innx);
					}
					wtx = weightX;
					fdx = foldX;
					x -= half;
					...
					for (i = x; (i < (x + width)); i++) {
					  ...
					  *fdx++ = (ptrdiff_t)fold(i, innx);
					}
					q = 0.0;
					wtz = weightZ;
					fdz = foldZ;
					for (k = (innz == 1L) ? (1L) : (width); (k-- > 0L);) {
					  wty = weightY;
					  fdy = foldY;
					  pj = splinePtr + *fdz++;
					  qj = 0.0;
		  ...
		  case 3:
			...
			if (directBsplineMirror(inPtr, splinePtr, innx, inny, innz, 3) == ERROR) {
			  freeVolF(splinePtr);
			  VipPrintfError("ERROR - Unable to compute B-spline coefficients");
			  return(ERROR);
			}
		  ...
		  case 4:
			...
			if (directBsplineMirror(inPtr, splinePtr, innx, inny, innz, 4) == ERROR) {
			  freeVolF(splinePtr);
			  VipPrintfError("ERROR - Unable to compute B-spline coefficients");
			  return(ERROR);
			}
		  ...
		  case 5:
			...
			if (directBsplineMirror(inPtr, splinePtr, innx, inny, innz, 5) == ERROR) {
			  freeVolF(splinePtr);
			  VipPrintfError("ERROR - Unable to compute B-spline coefficients");
			  return(ERROR);
			}
		  ...
		  case 6:
			...
			if (directBsplineMirror(inPtr, splinePtr, innx, inny, innz, 6) == ERROR) {
			  freeVolF(splinePtr);
			  VipPrintfError("ERROR - Unable to compute B-spline coefficients");
			  return(ERROR);
			}
		  ...
		  case 7:
			...
			if (directBsplineMirror(inPtr, splinePtr, innx, inny, innz, 7) == ERROR) {
			  freeVolF(splinePtr);
			  VipPrintfError("ERROR - Unable to compute B-spline coefficients");
			  return(ERROR);
			}
			...
			break;
		  default:
			VipPrintfError("ERROR - Unknown interpolation specification");
			return(ERROR);
		}
		return(!ERROR);
