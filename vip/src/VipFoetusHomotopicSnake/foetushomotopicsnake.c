...
{     
  ...
  */
  ...
  for (i=1;i<argc;i++)
      ...
      else if (!strncmp (argv[i], "-pressure", 2)) 
	{
	  ...
	  if(pressure<0 || pressure>100)
	    {
	      VipPrintfError("pressure should be in [0:100]");
	      return Usage();
	    }
	}
  ...
  VipChangeIntLabel(closing,255,mW+5*sW);
  ...
  /*this first dilation is to be robust to some vasculature or meninge staying alive
    ...
    VipHomotopicGeodesicDilation( pyrlab->image[0]->volume, 1,
    ...
    VipFoetusHomotopicInsideDilationSnake(pyrlab->image[0]->volume,pyr->image[0]->volume,
